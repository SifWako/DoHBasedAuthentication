...
app.config['PROPAGATE_EXCEPTIONS'] = False  # Set PROPAGATE_EXCEPTIONS to False
app.debug = False  # Set DEBUG to False
db = SQLAlchemy(app)
...
def handle_server_authentication():
    ...
    time.sleep(3)
    print(Fore.GREEN + f"2. GENERATING A RANDOM CHALLENGE" + Style.RESET_ALL)
    ...
    time.sleep(3)
    print(Fore.GREEN + f"3. SENT THE CHALLENGE {challenge} TO PROVE DOMAIN OWNERSHIP" + Style.RESET_ALL)
    time.sleep(3)
    threading.Thread(target=wait_and_verify, args=(domain,)).start()
    return jsonify({'challenge': challenge, 'instruction': f"https://{domain}/.well known/publickeyauth/{challenges[domain]}"})
...
def wait_and_verify(domain):
    ...
    for _ in tqdm(range(10), desc=f"{yellow}5. WAITING UNTIL THE SOLUTION IS PLACED{reset}"):
        time.sleep(1)
...
def verify_for_domain(domain):
    time.sleep(3)
    print(Fore.GREEN + f"6. VERIFY IF THE FILE IS REACHABLE ON DOMAIN {domain}" + Style.RESET_ALL)
    ...
    try:
        response = requests.get(f"https://localhost:3930/.well known/publickeyauth/{expected_solution}", verify="FinalExampleCert.pem")
        if response.text == expected_solution:
            time.sleep(3)
            print(f"FILE IS REACHABLE!!! THE WEB-SERVER {domain} IS AUTHENTICATED SUCCESSFULLY")
            ...
            try:
                ...
                expiration_message = f"EXPIRES ON {ttl_expiration_str}"
                ...
                with app.app_context():
                    ...
                    if existing_record:
                        ...
                        db.session.commit()
                        time.sleep(3)
                        print(Fore.GREEN + f"8. UPDATED EXISTING RECORD FOR DOMAIN {cn}." + Style.RESET_ALL)
                    else:
                        # Create a new record
                        ...
                        db.session.commit()
                        time.sleep(3)
                        print(f"8. STORED THE AUTHENTICATED PUBLIC KEY FOR {cn}. WITH EXPIRY DATE")
...
def send_verification_result(domain, success, ttl_expiration_str=None):
    ...
    try:
        ...
        print(f"SENT VERIFICATION RESULT FOR THE DOMAIN. Status: {response.status_code}, TTL Expiration: {ttl_expiration_str}")
    except requests.RequestException as e:
...
if __name__ == "__main__":
    ...
    # scheduler.add_job(func=check_and_revoke_expired_keys, trigger="interval", hours=0.0333333,
    #                   next_run_time=datetime.now())
    # scheduler.start()
    # atexit.register(lambda: scheduler.shutdown())

    print(Fore.BLUE + f"DOH SERVER LISTENING ON PORT {PORT}\n" + "✧" * 50 + Style.RESET_ALL)

    # Run the Flask app with use_reloader=False
    app.run(host='0.0.0.0', port=PORT, ssl_context=('DOHServerCertificate.pem', 'DOHServerPrivate_key.pem'),
